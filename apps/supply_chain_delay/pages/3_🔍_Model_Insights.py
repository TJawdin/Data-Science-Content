...
# Load metadata
try:
    metadata_path = Path(__file__).parent.parent / "artifacts" / "final_metadata.json"
...
try:
    results_path = Path(__file__).parent.parent / "artifacts" / "model_results.json"
...
with tab1:
    ...
    with col1:
        auc = metadata.get('best_model_auc', 0.85)
        st.metric(
            "AUC-ROC Score",
            f"{auc:.4f}",
    ...
    with col2:
        st.metric(
            "Model Type",
            metadata.get('best_model', 'XGBoost')
        )
    
    with col3:
        st.metric(
            "Features",
            metadata.get('n_features', 30)
        )
    
    with col4:
        st.metric(
            "Training Date",
            metadata.get('training_date', 'Oct 2025')
        )
    ...
    if model_results:
        ...
        st.dataframe(
            comparison_df.style.format({
                ...
                'Recall': '{:.4f}',
                'F1-Score': '{:.4f}'
            }).background_gradient(cmap='RdYlGn', subset=['AUC-ROC', 'Accuracy', 'Precision', 'Recall', 'F1-Score']),
    ...
    **What is Feature Importance?**
    ...
    if importance_df is not None:
        ...
        **Top 3 Factors Driving Late Deliveries:**
        ...
        1. **{feature_descriptions[top_3.iloc[0]['Feature']]}** (Importance: {top_3.iloc[0]['Importance']:.4f})
        2. **{feature_descriptions[top_3.iloc[1]['Feature']]}** (Importance: {top_3.iloc[1]['Importance']:.4f})
        3. **{feature_descriptions[top_3.iloc[2]['Feature']]}** (Importance: {top_3.iloc[2]['Importance']:.4f})
...
with tab3:
    ...
    **Why it matters:** Understanding correlations helps identify redundant features and 
    ...
    # Create sample correlation data (in production, this would come from training data)
    # For demo purposes, we'll create a realistic correlation matrix
    
    feature_names_short = [
        'num_items', 'total_order_value', 'total_shipping_cost', 'total_weight_g',
        'avg_shipping_distance_km', 'is_cross_state', 'is_multi_seller',
        'estimated_days', 'order_weekday', 'order_month'
    ]
    ...
    # num_items correlates with total_order_value
    corr_matrix[0, 1] = corr_matrix[1, 0] = 0.75
    # num_items correlates with total_weight
    corr_matrix[0, 3] = corr_matrix[3, 0] = 0.68
    # total_order_value correlates with shipping_cost
    corr_matrix[1, 2] = corr_matrix[2, 1] = 0.55
    # distance correlates with is_cross_state
    corr_matrix[4, 5] = corr_matrix[5, 4] = 0.72
    # distance correlates with shipping_cost
    corr_matrix[4, 2] = corr_matrix[2, 4] = 0.63
    
    corr_df = pd.DataFrame(
        corr_matrix,
        index=[get_feature_descriptions()[f] for f in feature_names_short],
        columns=[get_feature_descriptions()[f] for f in feature_names_short]
    )
...
with tab4:
    ...
    **Geographic patterns** reveal which regions have higher late delivery rates.
    ...
    # Sample Brazilian state data (from notebook EDA)
    # In production, this would come from actual training data
    state_data = {
    ...
    # Create dataframe
    geo_df = pd.DataFrame([
        {'State': code, 'State_Name': state_names[code], 'Late_Rate': rate}
        for code, rate in state_data.items()
    ...
    fig = go.Figure(go.Bar(
        ...
        marker=dict(
            color=top_10_states['Late_Rate'],
            colorscale='Reds',
            showscale=True,
            colorbar=dict(title="Late Rate (%)")
        ),
        hovertemplate='<b>%{x} - ' + top_10_states['State_Name'] + '</b><br>' +
                      'Late Rate: %{y:.1f}%<extra></extra>'
    ))
    ...
    fig2 = go.Figure(go.Bar(
        ...
        textposition='outside',
        marker=dict(
            color=bottom_10_states['Late_Rate'],
            colorscale='Greens',
            showscale=True,
            colorbar=dict(title="Late Rate (%)")
        ),
        hovertemplate='<b>%{x} - ' + bottom_10_states['State_Name'] + '</b><br>' +
                      'Late Rate: %{y:.1f}%<extra></extra>'
    ))
